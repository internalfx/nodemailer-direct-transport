...
function DirectMailer(options) {
    ...
    if (!this.options.hasOwnProperty('retryOnFailure')) {
      this.options.retryOnFailure = true;
    }
            ...
            this._process([].concat(exchanges), data, (function (err, response) {
                ...
                if (err) {
                    if (err.responseCode && err.responseCode >= 500) {
                        ...
                        console.log("RETRY ON FAILURE ==============================================================================" + this.options.retryOnFailure)
                        if (this.options.retryOnFailure && data.replies <= 5) {
                            console.log("RETRY ON FAILURE ========================================IT TRIED AGAIN!!!!======================================" + this.options.retryOnFailure)
